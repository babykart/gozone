...
func testTemplateSet() *template.Template {
	...
	return template.Must(template.New("test").Funcs(funcMap).Parse(`
		...
		{{define "activity.html"}}Activity: {{range .Logs}}{{.Action}} {{end}}{{if .PageInfo}} PageInfo={{.PageInfo.Current}}/{{.PageInfo.TotalPages}} Search={{.Search}}{{end}}{{if .Extra}} Extra={{.Extra}}{{end}}{{end}}
	`))
}

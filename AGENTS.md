...
- **Templates & static files**: embedded via `//go:embed` in `web/embed.go`, loaded with `template.ParseFS`; template FuncMap lives in `cmd/templates.go` (`parseTemplates`/`templateFuncMap`) and includes `add`, `sub`, `urlquery`, `relativeName`, `dict`, `assetVersion`. `assetVersion` returns a short content-hash of the bundled JS/CSS (`staticAssetVersion`) for cache-busting; bumping static assets flows through automatically.
...
- **Config**: YAML file + env var overrides with `GOZONE_` prefix. Default admin: `admin` / `admin` (override via `GOZONE_ADMIN_PASSWORD`). Without `server.secret_key` / `GOZONE_SECRET_KEY` a random key is generated per restart — invalidating all sessions and CSRF tokens; set it for stable local sessions. `server.trusted_proxies` entries **must be CIDR** (e.g. `10.0.0.0/8`, `192.0.2.1/32`) — a plain IP without `/` fails startup with an actionable validation error (`parseTrustedProxies` in `cmd/middleware.go`, surfaced through `runServer` before the database opens).
...
- **FuncMap sync**: the test FuncMap in `testTemplateSet()` is a **subset** of the real one in `cmd/templates.go:parseTemplates()`. If a new template func is added to `parseTemplates()`, it must also be added to `testTemplateSet()` or tests that render templates will fail.
...
- **`relativeName`** lives in `cmd/templates.go` (package cmd), so handler tests stub it out — the test FuncMap maps it to a no-op.
- **Admin routes**: every admin web route is registered in one place — `mountAdminRoutes` in `cmd/routes.go` — and `TestAdminRoutesProtectedByRequireAdmin` (in `cmd/routes_test.go`) walks that table to assert none escapes `middleware.RequireAdmin`. Add new admin routes to `mountAdminRoutes` so the invariant stays green.
